			...
			{links.map((item) => {
				return (
					<Link
						key={item.id}
						href={item.link}
						className="text-gray-500 hover:bg-lightBrown hover:text-white py-[10px] px-[15px] rounded-[10px]"
					>
						{item.title}
					</Link>
				);
...
const Navbar = () => {
	...
	return (
		<nav className="w-full bg-mainDark text-white font-anton text-[20px] tracking-wide py-[1.3rem] px-[1rem] md:px-[2rem] xl:px-0">
			<div className="container flex items-center justify-between">
						...
						{navbarLinks.map((navItem) => {
							...
							return (
								<li
									key={navItem.id}
									onMouseEnter={() => {
										handleNavEnter(navItem.id);
									}}
									className="relative"
								>
									<Link
										href={navItem.link}
										className="uppercase cursor-pointer nav-link flex items-center gap-[10px]"
									>
				...
				<div className="flex items-center gap-[10px]">
					...
					<Link
						href={"#"}
						className="hidden xl:inline-block font-anton text-white py-[8px] px-[20px] rounded-[10px] bg-lightBrown hover:bg-red-800 text-[12px] uppercase transition-colors duration-300 ease-in-out"
					>
						Play Now
					</Link>
				</div>
