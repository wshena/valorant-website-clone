...
export const PlusIcon = ({color, size, classname}:IconProps) => {
  return (
    <FiPlus color={color} size={size} className={classname} />
  )
}

export const MinusIcon = ({color, size, classname}:IconProps) => {
  return (
    <FiMinus color={color} size={size} className={classname} />
  )
}

export const Facebook = ({ size, color }: {size:number, color:string}) => {
	return <FaFacebook size={size} color={color} />;
};

export const Youtube = ({ size, color }: {size:number, color:string}) => {
	return <FaYoutube size={size} color={color} />;
};

export const Instagram = ({ size, color }: {size:number, color:string}) => {
	return <FaInstagram size={size} color={color} />;
};

export const ArrowNorthEast = ({color, size, classname}:IconProps) => {
  return (
    <MdArrowOutward color={color} size={size} className={classname} />
  )
}

export const GlobeIcon = ({color, size, classname}:IconProps) => {
  return (
    <CiGlobe color={color} size={size} className={classname} />
  )
}

export const ArrowUpIcon = ({color, size, classname}:IconProps) => {
  return (
    <IoMdArrowDropup color={color} size={size} className={classname} />
  )
}

export const ArrowDownIcon = ({color, size, classname}:IconProps) => {
  return (
    <IoMdArrowDropdown color={color} size={size} className={classname} />
  )
}

export const RiotLogo = ({fillColor, width, height, classname}:{fillColor:string, width:string, height:string, classname:string}) => {
  return (
    <svg width={width} height={height} viewBox="0 0 587.93 165" fill={`${fillColor}`} className={classname} >
...
export const ValorantLogo = ({width, height, classname}:{width:string, height:string, classname:string}) => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" height={height} viewBox="0 0 100 100" width={width} className={classname}><path d="M99.25 48.66V10.28c0-.59-.75-.86-1.12-.39l-41.92 52.4a.627.627 0 00.49 1.02h30.29c.82 0 1.59-.37 2.1-1.01l9.57-11.96c.38-.48.59-1.07.59-1.68zM1.17 50.34L32.66 89.7c.51.64 1.28 1.01 2.1 1.01h30.29c.53 0 .82-.61.49-1.02L1.7 9.89c-.37-.46-1.12-.2-1.12.39v38.38c0 .61.21 1.2.59 1.68z" fill="#fff"></path></svg>
  )
}